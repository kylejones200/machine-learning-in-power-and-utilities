...
canonical_link: "https://medium.com/@kyle-t-jones/machine-learning-in-power-and-utilities-8a5aa9aa3341"
---
...
In this tutorial, we'll generate synthetic load data influenced by daily temperature cycles and random noise, mirroring real-world patterns. We'll then fit a linear regression model to explain and predict load trends. This exercise introduces:
...
While simplistic, this forms the foundation for later tutorials: ARIMA time series, predictive maintenance, and reinforcement learning for grid control.
...
By the end, you'll see how a straightforward ML model maps directly to a utility use case --- predicting load shifts driven by weather --- and understand why even simple analytics can drive measurable value.

ML in utilities isn't magic; it's about pairing domain knowledge with the right data and techniques to solve operational problems.

In later posts, we will layer on complexity: hourly AMI data, SCADA sensor feeds, vegetation risk models, YOLO-based drone inspection, LLMs for log analysis, and eventually full-scale AI-driven utility platforms. But we start here --- with a single, interpretable model connecting a core variable (temperature) to a critical output (load).
